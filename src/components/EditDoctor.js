...
const EditDoctor = (props) => {
    ...
    const [updatedPost, setUpdatedPost] = useState({
        ...
        specialisation: props.specialisation
    });
    ...
    const updateDoctorId = (e) => {
        let value = e.target.value;
        setUpdatedPost({ ...updatedPost, doctorId: value });
        // console.log(updatedPost);
    }

    const updateSpecialisation = (e) => {
        let value = e.target.value;
        setUpdatedPost({ ...updatedPost, specialisation: value });
        // console.log(updatedPost);
    }
    ...
    const updatePost = (e) => {
        ...
        axios.post('http://localhost:80/apiMain/updateDoctor.php', updatedPost)
            .then((res) => {
                let data = res.data;
                console.log(data);
                props.upRender(true);
    ...
    return (
        ...
        <div className="edit-con" >
            ...
            <div className='add-con add-space'>
                <p className='add-name-heading-edit'>Doctor Id</p>
                <input className='name-input-pat-edit' placeholder={props.doctorId} onChange={updateDoctorId}></input>
            </div>
            <div className='add-con add-space'>
                <p className='add-name-heading-edit'>Specialisation</p>
                <input className='name-input-pat-edit' placeholder={props.specialisation} onChange={updateSpecialisation}></input>
            </div>
